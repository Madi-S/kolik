...
@router.get(
    "/", response_model=List[UserOut], tags=["user"], status_code=status.HTTP_200_OK
)
...
@router.get(
    "/{id}", response_model=UserOut, tags=["user"], status_code=status.HTTP_200_OK
)
async def get_user_by_id(request: Request, response: Response, id: str = Path(...)):
    if user := User.query.get(id):
        return user

    raise HTTPException(status.HTTP_404_NOT_FOUND, messages.USER_NOT_FOUND.format(id))


@router.post("/{id}", tags=["user"], status_code=status.HTTP_204_NO_CONTENT)
@limiter.limit("2/minute")
async def edit_user(
    request: Request, response: Response, data: UserEditIn, id: str = Path(...)
):
    if user := User.query.get(id):
        user.edit(data.dict())
    else:
        raise HTTPException(
            status.HTTP_404_NOT_FOUND, messages.USER_NOT_FOUND.format(id)
        )
...
async def send_confirmation_code(
    ...
    if phone_entity.confirmation_code_sent(code):
        phone_obj = Phone.query.filter_by(value=phone).first() or Phone.create(phone)
...
async def create_user(
    ...
    if phone_obj := Phone.query.filter_by(value=data.phone).first():
        if phone_obj.confirmed(confirmation_code):
            ...
            user = User.query.filter_by(phone=data.phone).first() or User.create(
                data.dict()
            )
            return user

        raise HTTPException(
            status.HTTP_401_UNAUTHORIZED, messages.CONFIRMATION_CODE_IS_INCORRECT
        )

    raise HTTPException(status.HTTP_404_NOT_FOUND, messages.PHONE_NOT_FOUND)

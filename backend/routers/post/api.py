...
router = APIRouter(
    prefix="/post", tags=["post"], dependencies=[Depends(validate_auth_token)]
)
...
@router.post(
    "/query/count", response_model=int, tags=["post"], status_code=status.HTTP_200_OK
)
...
@router.get(
    "/{id}", response_model=PostOut, tags=["post"], status_code=status.HTTP_200_OK
)
async def get_post_by_id(request: Request, response: Response, id: str = Path(...)):
...
async def get_all_posts_by_user_id(
    request: Request, response: Response, user_id: str = Path(...)
):
...
@router.put(
    "/", response_model=PostOut, tags=["post"], status_code=status.HTTP_201_CREATED
)
@limiter.limit("3/minute")
async def create_post(request: Request, response: Response, data: PostIn):
...
async def delete_post(request: Request, response: Response, id: str = Path(...)):
    post_was_deleted = Post.delete(id)
    if not post_was_deleted:
        raise HTTPException(
            status.HTTP_404_NOT_FOUND, messages.POST_NOT_FOUND.format(id)
        )
...
async def edit_post(
    request: Request, response: Response, data: PostEditIn, id: str = Path(...)
):
    if post := Post.query.get(id):
...
@router.post(
    "/activate/{id}", response_model=bool, tags=["post"], status_code=status.HTTP_200_OK
)
@limiter.limit("4/minute")
def activate_post(request: Request, response: Response, id: str = Path(...)):
...
@limiter.limit("4/minute")
def deactivate_post(request: Request, response: Response, id: str = Path(...)):
...
@router.put(
    "/image/{post_id}", tags=["post", "image"], status_code=status.HTTP_204_NO_CONTENT
)
